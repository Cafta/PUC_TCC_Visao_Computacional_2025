...
class HandDetector:
    ...
    def get_roi(self, frame, hand_position, arm_length=None):
        ...
        if arm_length and arm_length > 0:
            # Use 2x arm_length as base size (normalized 0-1 to pixels)
            roi_size = int(arm_length * max(w, h) * 2.0)
        else:
            # Default starting size - 25% of smaller frame dimension
            roi_size = int(min(w, h) * 0.25)
            
        # 2. Adjust by previous hand size if we had a valid detection
        if hasattr(self, 'last_hand_size') and self.last_hand_size:
            # Blend between arm-based and hand-based size (75% hand, 25% arm)
            hand_based_size = int(self.last_hand_size * 1.5)  # 50% margin
            roi_size = int(0.25 * roi_size + 0.75 * hand_based_size)
            
        # 3. Enforce size limits
        roi_size = max(100, min(roi_size, min(w, h)))  # Between 100px and frame
    ...
    def detect(self, frame, roi=None, arm_length=None):
        ...
        if self.hand_detected:
            ...
            # Calcula tamanho da mão baseado na distância entre pulso e ponta do dedo médio
            wrist = hand_landmarks.landmark[0]  # Pulso
            middle_tip = hand_landmarks.landmark[12]  # Ponta do dedo médio
            hand_width_px = abs(middle_tip.x - wrist.x) * roi_w
            hand_height_px = abs(middle_tip.y - wrist.y) * roi_h
            hand_size = np.sqrt(hand_width_px**2 + hand_height_px**2)
