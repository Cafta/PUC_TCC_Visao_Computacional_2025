...
class HandDetector:
    ...
    def get_roi(self, frame, hand_position, arm_length=None):
        ...
        """
        h, w = frame.shape[:2]
        x, y = int(hand_position[0] * w), int((hand_position[1]+arm_length) * h)
        
        # Se temos arm_length, usa ele para ajustar o tamanho do ROI
        # Caso contrário, usa tamanho padrão
        if arm_length: print(f"arm_length = {arm_length}")
        if arm_length and arm_length > 0:
            ...
            roi_size = int(arm_length * max(w, h) * 2.5)  # 2.5x o comprimento do antebraço
    ...
    def calculate_openness(self, landmarks, arm_length=None):
        ...
        if arm_length is None or arm_length <= 0.001:
            ...
            arm_length = np.sqrt(
                ((middle_base.x * Config.FRAME_WIDTH) - (wrist.x * Config.FRAME_HEIGHT))**2 + 
                ((middle_base.y * Config.FRAME_WIDTH) - (wrist.y * Config.FRAME_HEIGHT))**2
            )
