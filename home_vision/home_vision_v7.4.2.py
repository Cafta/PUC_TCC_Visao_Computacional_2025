...
class HandDetector:
    ...
    def detect(self, frame, roi=None, arm_length=None):
        ...
        if self.hand_detected:
            ...
            # Se não foi passado arm_length, estima usando landmarks e o tamanho do ROI
            if arm_length is None or arm_length <= 0.001:
                # usa dimensões do ROI original (antes do zoom) para estimativa
                roi_w = x2 - x1
                roi_h = y2 - y1
                ...
                # normaliza pela maior dimensão do ROI
                arm_length_est = arm_length_pixels / max(roi_w, roi_h) if max(roi_w, roi_h) > 0 else 0.1
                self.hand_openness = self.calculate_openness(hand_landmarks.landmark, arm_length_est)
            else:
